...
def build_bottom():
    ...
    s=s.cut(rect(CAV_W,CAV_H,-8.50,BOTTOM_TOP+8.50+.55))
    for x,y in MOUNT_HOLES:
        s=s.union(cyl(8.2,-6.05,5.27,x,y)).cut(cyl(4.7,-5.08,4.05,x,y))
    s=s.cut(rect(12.4,24.0,-6.1,6.0,-.465,-53.0))
    for sx in (-1,1):
        x=sx*54.0
        for yy in (-26,0,26):
            try:
                port=cq.Workplane("YZ",origin=(x,0,0)).circle(3.2).extrude(4.0*sx).translate((0,yy,-4.0))
                s=s.cut(port)
            except Exception:
                pass
    return s
...
def build_top():
    ...
    top=top.cut(rect(*OLED_WINDOW,TOP_Z0-.5,TOP_H+1.0,*OLED_CENTER))
    top=top.cut(cyl(ENC_OPEN,TOP_Z0-.5,TOP_H+1.0,*ENC_CENTER))
    for x,y in MOUNT_HOLES:
        top=top.cut(cyl(3.4,TOP_Z0-.5,TOP_H+1.0,x,y)).cut(cyl(5.8,TOP_Z1-1.25,1.30,x,y))

    RD,RZ,IH,IZ=.58,TOP_Z1-.58,.48,TOP_Z1-.48-.04
    ins=[]
    top=top.cut(rect_ring(34.2,15.2,31.2,12.2,RZ,RD+.1,*OLED_CENTER))
    ...
    top=top.cut(er); ins.append(eri.val())

    for x,y in SWITCH_CENTERS:
        top=top.cut(annulus(18.0,16.55,RZ,RD+.1,x,y)); ins.append(annulus(17.72,16.87,IZ,IH,x,y).val())

    for x,y,w,h,a in [(-45,-12,2.8,31,0),(45,-12,2.8,31,0),(-43,22,2.8,18,0),(43,22,2.8,18,0),(-28,-52,15,2.6,0),(28,-52,15,2.6,0)]:
        r=keepout(rotated_rect(w,h,a,RZ,RD+.1,x,y),RZ,RD+.1)
        i=keepout(rotated_rect(max(w-.35,.8),max(h-.35,.8),a,IZ,IH,x,y),IZ,IH,7.2,21.3,(35.8,16.8))
        top=top.cut(r)
        if i.solids().vals(): ins.append(i.val())

    # Celestial details
    top=top.cut(crescent(5.35,4.55,2.45,RZ,RD+.1,-42.5,43.5)); ins.append(crescent(5.02,4.28,2.30,IZ,IH,-42.5,43.5).val())

    # Main planet between OLED and encoder
    r=keepout(planet(5.0,5.7,1.95,4.25,.95,16,RZ,RD+.1,7.8,41.0),RZ,RD+.1,7.0,20.5,(34.8,15.8))
    i=keepout(planet(4.72,5.35,1.76,4.02,.86,16,IZ,IH,7.8,41.0),IZ,IH,7.2,20.8,(35.1,16.1))
    top=top.cut(r)
    if i.solids().vals(): ins.append(i.val())

    # FINAL FIX: right planet sits in the clean gap between the two right reactor rails.
    top=top.cut(planet(3.8,4.5,1.55,3.3,.72,-20,RZ,RD+.1,49.0,8.5)); ins.append(planet(3.55,4.2,1.40,3.10,.64,-20,IZ,IH,49.0,8.5).val())
    top=top.cut(planet(3.4,4.0,1.35,2.95,.62,26,RZ,RD+.1,-47.0,-31.0)); ins.append(planet(3.18,3.72,1.22,2.78,.56,26,IZ,IH,-47.0,-31.0).val())

    sd=[(-47,24.5,.92),(-37,31,.62),(-3,52,.52),(18,53.8,.68),(47,30.5,.72),(51,-30,.54),(-52,-7,.46),(-12,-53,.44),(12,-54,.44),(-18.5,18,.46),(18.5,18,.46)]
    top=top.cut(cq.Workplane(obj=stars(sd,RZ,RD+.1))); ins.append(stars([(x,y,max(s-.18,.32)) for x,y,s in sd],IZ,IH))

    # Purple LTC planet with black LTC revealed through it.
    bp=planet(18.6,14.8,4.7,11.6,2.65,-12,RZ,RD+.1,0,-44)
    bi=planet(18.1,14.35,4.38,11.3,2.45,-12,IZ,IH,0,-44)
    bp=bp.cut(text_solid("LTC",8.1,RD+.3,0,-44,RZ-.1)); bi=bi.cut(text_solid("LTC",7.9,IH+.3,0,-44,IZ-.1))
    top=top.cut(bp); ins.append(bi.val())
    ld=[(-18,-46.5,.82),(17.5,-48,.62),(-15,-38,.52),(15.5,-37.5,.72),(-24,-54,.45),(24,-54,.45)]
    top=top.cut(cq.Workplane(obj=stars(ld,RZ,RD+.1))); ins.append(stars([(x,y,max(s-.16,.30)) for x,y,s in ld],IZ,IH))
...
def render_parts():
    ...
    knob=cyl(15.8,6.55,11.3,*ENC_CENTER).union(cyl(15.0,17.85,.9,*ENC_CENTER))
    kring=annulus(18.7,16.2,5.88,.9,*ENC_CENTER)
    screws=[]
    for x,y in MOUNT_HOLES:
        screws.extend([cyl(2.9,TOP_Z0-.4,10.2,x,y).val(),cyl(5.5,TOP_Z1-.95,1.6,x,y).val()])
    return caps,pcb,bezel,screen,oledtxt,knob,kring,cq.Compound.makeCompound(screws)


def mesh(shape,name,color,tmp):
    p=tmp/f"{name}.stl"; exporters.export(shape,str(p)); m=trimesh.load_mesh(p,force="mesh")
...
for name,shape in [("Top.STEP",top),("Bottom.STEP",bottom),("Middle_Purple.STEP",spacer),("Purple_Inlays.STEP",cq.Workplane(obj=inlays)),
    ...
    exporters.export(shape,str(PROD/name))
...
with tempfile.TemporaryDirectory() as td:
    td=Path(td); scene=trimesh.Scene()
    for s,n,c in [
        ...
        (knob,"Encoder_Knob",[15,15,22,255]),(kring,"Purple_Knob_Ring",[183,67,255,255]),(cq.Workplane(obj=screws),"M3_Screws",[175,175,190,255])]:
...
check={
    ...
    "m3_pass_through_mm":3.4,"screw_head_seat_mm":5.8,"heat_set_insert_pocket_mm":4.7,"encoder_opening_mm":ENC_OPEN,
