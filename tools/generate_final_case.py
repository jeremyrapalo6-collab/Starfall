...
def build_bottom():
    ...
    s=s.cut(rect(18.0,24.0,-7.10,8.0,-.465,-53.0))
    for sx in (-1,1):
        for yy in (-26,0,26):
...
def build_top():
    ...
    # Celestial details
    ins.append(crescent(5.02,4.28,2.30,IZ,IH,-42.5,43.5).val())
    ...
    ins.append(planet(3.55,4.2,1.40,3.10,.64,-20,IZ,IH,49.0,8.5).val())
    ins.append(planet(3.18,3.72,1.22,2.78,.56,26,IZ,IH,-47.0,-31.0).val())

    sd=[(-47,24.5,.92),(-37,31,.62),(-3,52,.52),(18,53.8,.68),(47,30.5,.72),(51,-30,.54),(-52,-7,.46),(-12,-53,.44),(12,-54,.44),(-18.5,18,.46),(18.5,18,.46)]
    ins.append(stars([(x,y,max(s-.18,.32)) for x,y,s in sd],IZ,IH))
    ...
    ins.append(stars([(x,y,max(s-.16,.30)) for x,y,s in ld],IZ,IH))
